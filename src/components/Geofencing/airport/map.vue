...
<template>
<div>
  <div id="map" style="width:90%; border:none; height:400px">
  </div> <br/>
  
           <center>
              
              <button @click.prevent="drawPolygon()" style="color:white;width:5%; margin-right: 2%;">
                <i class="fa fa-pencil" aria-hidden="true"></i>
              </button>               
              <button @click="deleteSelectedShape()" style="color:yellow;width:5%; margin-right: 2%;">
                <i class="fa fa-times" aria-hidden="true"></i>
              </button> 
              <button @click="clearMap()" style="color:red;width:5%; margin-right: 2%;">
...
export default {
  ...
  data() {
    return {
      location:this.locarea.name != "Select Area " ? this.locarea.name :'Nigeria',
      drawingMode:null,
      selectedShape:null,
      selectedKernel:null,
      gmarkers:[],
  ...
  methods: {       
    ...
    clearMap() {
        if (this.allShapes.length > 0) { // verific daca exista forme desenate
            for (var i = 0; i < this.allShapes.length; i++) // sterge toate formele
            {
...
findAvg(Coords) {
   ...
   async loadMap(){
      ...
      google.maps.event.addListener(self.drawingManager, 'overlaycomplete', function(e) {
        ...
        self.$emit('coordinates_points', JSON.stringify(lat_lng));
